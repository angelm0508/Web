$(function () {
    const tabla = $('#tblPedidosCompra').DataTable({
        ajax: { url: '/PedidosCompra/ObtenerTodos', dataSrc: App.dataSrcTabla },
        columns: [
            { data: 'numDoc' },
    ...
    function inicializarBuscadorSocio() {
        ...
        App.autocompletar({
            ...
            error: $('#CodigoSnError'),
            endpoint: '/PedidosCompra/BuscarSocios',
    ...
    $('#btnNuevo').on('click', async function () {
        const html = await $.get('/PedidosCompra/FormularioCrear');
        abrirModal(html);
    });

    $('#tblPedidosCompra').on('click', '.btn-editar', async function () {
        const entry = $(this).data('entry');
        const html = await $.get('/PedidosCompra/FormularioEditar', { entry });
        abrirModal(html);
    });

    $('#tblPedidosCompra').on('click', '.btn-eliminar', async function () {
        const entry = $(this).data('entry');
        const confirmado = await App.confirmarEliminar(`Se eliminará el pedido de compra #${entry}.`);
        if (!confirmado) return;

        const respuesta = await App.eliminar(`/PedidosCompra/Eliminar?entry=${entry}`);
        if (!respuesta.resultado) {
            App.mostrarError(respuesta.mensaje);
            return;
        }
        App.mostrarExito('Pedido de compra eliminado correctamente.');
        recargarTabla();
    });

    // --- Serie de numeración para generar el número de documento (solo aplica al crear) ---

    function inicializarSeriePedidoCompra() {
        const $sel = $('#selectSeriePedidoCompra');
        if ($sel.length === 0) return;

        const datosEl = document.getElementById('datosSeriesPedidoCompra');
    ...
    function esSerieManualPedidoCompra() {
        const $sel = $('#selectSeriePedidoCompra');
    ...
    function actualizarNumDocSegunSerie() {
        ...
        if (esSerieManualPedidoCompra()) {
            $numDoc.val('').prop('disabled', false).attr('placeholder', '');
        } else {
            const sigNumero = $('#selectSeriePedidoCompra').find('option:selected').data('sig-numero');
            $numDoc.val(sigNumero ?? '').prop('disabled', true).attr('placeholder', 'Se generará al guardar');
        }
    }

    $(document).on('change', '#selectSeriePedidoCompra', actualizarNumDocSegunSerie);

    $(document).on('click', '#btnGuardarPedidoCompra', async function () {
        const $boton = $(this);
        const esEdicion = $boton.data('edicion') === true || $boton.data('edicion') === 'true';
        const entry = $boton.data('entry');

        if (!esEdicion) {
            const serieSeleccionada = $('#selectSeriePedidoCompra').val();
        ...
        // servidor lo calcula y avanza el consecutivo al registrar el pedido de compra (ver
        // PedidoCompraDomain.InsertarAsync en la API), no antes. Para series Manual, el campo #NumDoc
        // está habilitado y su valor viaja normalmente en recolectarFormulario.
        const datos = App.recolectarFormulario('#formPedidoCompra');
        if (!esEdicion) {
            datos.Serie = $('#selectSeriePedidoCompra').val();
        }
        ...
        if (!esEdicion) {
            const respuestaCabecera = await App.enviarJson('/PedidosCompra/Crear', 'POST', datos);
            ...
            for (const linea of lineasLocales) {
                ...
                const respuestaLinea = await App.enviarJson('/PedidosCompra/CrearLinea', 'POST', {
                    ...lineaSinId,
                    Entry: entryCreado
                });
            ...
            if (fallidas > 0) {
                await App.mostrarExito(`Pedido de compra creado correctamente. Líneas guardadas: ${exitosas} de ${exitosas + fallidas}.${sufijoNumDoc}`);
            } else {
                await App.mostrarExito(`Pedido de compra creado correctamente.${sufijoNumDoc}`);
            }
        ...
        const respuesta = await App.enviarJson(`/PedidosCompra/Editar?entry=${entry}`, 'POST', datos);
        if (!respuesta.resultado) {
            App.mostrarError(respuesta.mensaje);
            return;
        }
        ...
        bootstrap.Modal.getInstance(document.getElementById('modalFormulario')).hide();
        App.mostrarExito('Pedido de compra actualizado correctamente.');
        recargarTabla();
    });
    ...
    function esEdicionDetalle() {
        const v = $('#tblDetallePedidoCompra').data('es-edicion');
    ...
    function inicializarDetalle() {
        ...
        const $tabla = $('#tblDetallePedidoCompra');
        ...
        buscadorArticuloCod = App.autocompletar({
            ...
            endpoint: '/PedidosCompra/BuscarArticulosPorCodigo',
        ...
        buscadorArticuloDesc = App.autocompletar({
            ...
            endpoint: '/PedidosCompra/BuscarArticulos',
            obtenerCodigo: a => a.codigo ?? a.Codigo,
        ...
        buscadorAlmacen = App.autocompletar({
            ...
            endpoint: '/PedidosCompra/BuscarAlmacenes',
        ...
        buscadorImpuesto = App.autocompletar({
            ...
            endpoint: '/PedidosCompra/BuscarImpuestos',
    ...
    async function cargarDetalleRemoto() {
        const entry = $('#tblDetallePedidoCompra').data('entry');
        const respuesta = await $.get('/PedidosCompra/ObtenerDetalle', { entry });
    ...
    function pintarDetalle() {
        const $tbody = $('#tblDetallePedidoCompra tbody');
    ...
    async function aplicarAlmacenDefectoDeArticulo(a) {
        ...
        const r = await $.get('/PedidosCompra/ObtenerAlmacenPorCodigo', { codigo: cod });
    ...
    $(document).on('click', '.btn-editar-linea', async function () {
        ...
        if (codAlmacen) {
            const respuestaAlmacen = await $.get('/PedidosCompra/ObtenerAlmacenPorCodigo', { codigo: codAlmacen });
        ...
        if (codigoImpuesto) {
            const respuestaImpuesto = await $.get('/PedidosCompra/ObtenerImpuestoPorCodigo', { codigo: codigoImpuesto });
    ...
    $(document).on('click', '.btn-eliminar-linea', async function () {
        ...
        if (esEdicionDetalle()) {
            const entry = $('#tblDetallePedidoCompra').data('entry');
            const respuesta = await App.eliminar(`/PedidosCompra/EliminarLinea?entry=${entry}&noLinea=${clave}`);
            if (!respuesta.resultado) {
                App.mostrarError(respuesta.mensaje);
                return;
            }
    ...
    $(document).on('click', '#btnGuardarLinea', async function () {
        ...
        if (esEdicionDetalle()) {
            ...
            const url = esEdicionLinea
                ? `/PedidosCompra/EditarLinea?entry=${entry}&noLinea=${noLineaOriginalEnEdicion}`
                : '/PedidosCompra/CrearLinea';

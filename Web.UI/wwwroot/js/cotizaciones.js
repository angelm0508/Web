$(function () {
    const tabla = $('#tblCotizaciones').DataTable({
        ajax: { url: '/Cotizaciones/ObtenerTodos', dataSrc: App.dataSrcTabla },
        columns: [
            { data: 'numDoc' },
    ...
    $('#btnNuevo').on('click', async function () {
        const html = await $.get('/Cotizaciones/FormularioCrear');
        abrirModal(html);
    });

    $('#tblCotizaciones').on('click', '.btn-editar', async function () {
        const entry = $(this).data('entry');
        const html = await $.get('/Cotizaciones/FormularioEditar', { entry });
        abrirModal(html);
    });

    $('#tblCotizaciones').on('click', '.btn-eliminar', async function () {
        const entry = $(this).data('entry');
        const confirmado = await App.confirmarEliminar(`Se eliminará la cotización #${entry}.`);
        if (!confirmado) return;

        const respuesta = await App.eliminar(`/Cotizaciones/Eliminar?entry=${entry}`);
        if (!respuesta.resultado) {
            App.mostrarError(respuesta.mensaje);
            return;
        }
        App.mostrarExito('Cotización eliminada correctamente.');
        recargarTabla();
    });

    // --- Serie de numeración para generar el número de documento (solo aplica al crear) ---

    function inicializarSerieCotizacion() {
        const $sel = $('#selectSerieCotizacion');
    ...
    function esSerieManualCotizacion() {
        const $sel = $('#selectSerieCotizacion');
    ...
    $(document).on('click', '#btnGuardarCotizacion', async function () {
        ...
        if (!esEdicion) {
            const serieSeleccionada = $('#selectSerieCotizacion').val();
        ...
        // El número de documento (No. documento) ya no se solicita aquí para series no
        // manuales: el servidor lo calcula y avanza el consecutivo al registrar la cotización
        // (ver CotizacionDomain.InsertarAsync en la API), no antes. Para series Manual, el campo
        // #NumDoc está habilitado y su valor viaja normalmente en recolectarFormulario.
        const datos = App.recolectarFormulario('#formCotizacion');
        if (!esEdicion) {
            datos.Serie = $('#selectSerieCotizacion').val();
        }
        ...
        if (!esEdicion) {
            const respuestaCabecera = await App.enviarJson('/Cotizaciones/Crear', 'POST', datos);
            ...
            // Muestra en el campo el No. documento real recién asignado por el servidor (para
            // series no manuales, antes solo decía "Se generará al guardar" y nunca se llegaba
            // a ver el número final aquí).
            ...
            for (const linea of lineasLocales) {
                ...
                const respuestaLinea = await App.enviarJson('/Cotizaciones/CrearLinea', 'POST', {
                    ...lineaSinId,
                    Entry: entryCreado
                });
            ...
            // El mensaje de éxito se espera (await) antes de cerrar el modal para que el usuario
            // alcance a ver el campo "No. documento" ya con el número real asignado.
            const sufijoNumDoc = respuestaCabecera.numDoc != null ? ` No. documento: ${respuestaCabecera.numDoc}.` : '';
            if (fallidas > 0) {
                await App.mostrarExito(`Cotización creada correctamente. Líneas guardadas: ${exitosas} de ${exitosas + fallidas}.${sufijoNumDoc}`);
            } else {
                await App.mostrarExito(`Cotización creada correctamente.${sufijoNumDoc}`);
            }
        ...
        const respuesta = await App.enviarJson(`/Cotizaciones/Editar?entry=${entry}`, 'POST', datos);
        if (!respuesta.resultado) {
            App.mostrarError(respuesta.mensaje);
            return;
        }
        ...
        bootstrap.Modal.getInstance(document.getElementById('modalFormulario')).hide();
        App.mostrarExito('Cotización actualizada correctamente.');
        recargarTabla();
    });
    ...
    function inicializarDetalle() {
        ...
        const datosArt = document.getElementById('datosArticulosCotizacion');
        articulosDisponibles = datosArt ? (JSON.parse(datosArt.textContent) || []) : [];

        const datosImp = document.getElementById('datosImpuestosCotizacion');
    ...
    async function cargarDetalleRemoto() {
        const entry = $('#tblDetalleCotizacion').data('entry');
        const respuesta = await $.get('/Cotizaciones/ObtenerDetalle', { entry });
    ...
    function pintarDetalle() {
        const $tbody = $('#tblDetalleCotizacion tbody');
    ...
    $(document).on('click', '.btn-eliminar-linea', async function () {
        ...
        if (esEdicionDetalle()) {
            const entry = $('#tblDetalleCotizacion').data('entry');
            const respuesta = await App.eliminar(`/Cotizaciones/EliminarLinea?entry=${entry}&noLinea=${clave}`);
            if (!respuesta.resultado) {
                App.mostrarError(respuesta.mensaje);
                return;
            }
    ...
    $(document).on('click', '#btnGuardarLinea', async function () {
        ...
        if (esEdicionDetalle()) {
            ...
            const url = esEdicionLinea
                ? `/Cotizaciones/EditarLinea?entry=${entry}&noLinea=${noLineaOriginalEnEdicion}`
                : '/Cotizaciones/CrearLinea';

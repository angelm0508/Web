$(function () {
    const tabla = $('#tblSalidasMercancia').DataTable({
        ajax: { url: '/SalidasMercancia/ObtenerTodos', dataSrc: App.dataSrcTabla },
        columns: [
            { data: 'numDoc' },
    ...
    function abrirModal(html) {
        ...
        inicializarSerieSalidaMercancia();
        inicializarDetalle();
    }

    $('#btnNuevo').on('click', async function () {
        const html = await $.get('/SalidasMercancia/FormularioCrear');
        abrirModal(html);
    });

    $('#tblSalidasMercancia').on('click', '.btn-editar', async function () {
        const entry = $(this).data('entry');
        const html = await $.get('/SalidasMercancia/FormularioEditar', { entry });
        abrirModal(html);
    });

    $('#tblSalidasMercancia').on('click', '.btn-eliminar', async function () {
        const entry = $(this).data('entry');
        const confirmado = await App.confirmarEliminar(`Se eliminará la salida de mercancía #${entry}.`);
        if (!confirmado) return;

        const respuesta = await App.eliminar(`/SalidasMercancia/Eliminar?entry=${entry}`);
        if (!respuesta.resultado) {
            App.mostrarError(respuesta.mensaje);
            return;
        }
        App.mostrarExito('Salida de mercancía eliminada correctamente.');
        recargarTabla();
    });

    $(document).on('click', '#btnCancelarDocSalidaMercancia', async function () {
        const entry = $(this).data('entry');
        const confirmado = await App.confirmarEliminar('Se cancelará este documento y se revertirá el inventario que descontó. Esta acción no se puede deshacer.');
        if (!confirmado) return;

        const $btn = $(this).prop('disabled', true);
        try {
            const respuesta = await App.enviarJson(`/SalidasMercancia/Editar?entry=${entry}`, 'POST', { Cancelado: 'S' });
            if (!respuesta.resultado) {
                App.mostrarError(respuesta.mensaje);
                return;
            }
    ...
    function inicializarSerieSalidaMercancia() {
        const $sel = $('#selectSerieSalidaMercancia');
        if ($sel.length === 0) return;

        const datosEl = document.getElementById('datosSeriesSalidaMercancia');
    ...
    function esSerieManualSalidaMercancia() {
        const $sel = $('#selectSerieSalidaMercancia');
    ...
    function actualizarNumDocSegunSerie() {
        ...
        if (esSerieManualSalidaMercancia()) {
            $numDoc.val('').prop('disabled', false).attr('placeholder', '');
        } else {
            const sigNumero = $('#selectSerieSalidaMercancia').find('option:selected').data('sig-numero');
            $numDoc.val(sigNumero ?? '').prop('disabled', true).attr('placeholder', 'Se generará al guardar');
        }
    }

    $(document).on('change', '#selectSerieSalidaMercancia', actualizarNumDocSegunSerie);

    $(document).on('click', '#btnGuardarSalidaMercancia', async function () {
        const $boton = $(this);
        const esEdicion = $boton.data('edicion') === true || $boton.data('edicion') === 'true';
        const entry = $boton.data('entry');

        if (!esEdicion) {
            const serieSeleccionada = $('#selectSerieSalidaMercancia').val();
        ...
        // servidor lo calcula y avanza el consecutivo al registrar la salida de mercancía, no
        // antes. Para series Manual, el campo #NumDoc está habilitado y su valor viaja normalmente
        // en recolectarFormulario.
        const datos = App.recolectarFormulario('#formSalidaMercancia');
        if (!esEdicion) {
            datos.Serie = $('#selectSerieSalidaMercancia').val();
        }
        ...
        if (!esEdicion) {
            ...
            const respuesta = await App.enviarJson('/SalidasMercancia/Crear', 'POST', datos);
            if (!respuesta.resultado) {
                App.mostrarError(respuesta.mensaje);
                return;
            }
            ...
            await App.mostrarExito(`Salida de mercancía creada correctamente.${sufijoNumDoc}`);
            bootstrap.Modal.getInstance(document.getElementById('modalFormulario')).hide();
            recargarTabla();
            return;
        }

        const respuesta = await App.enviarJson(`/SalidasMercancia/Editar?entry=${entry}`, 'POST', datos);
        if (!respuesta.resultado) {
            App.mostrarError(respuesta.mensaje);
            return;
        }
        ...
        bootstrap.Modal.getInstance(document.getElementById('modalFormulario')).hide();
        App.mostrarExito('Salida de mercancía actualizada correctamente.');
        recargarTabla();
    });
    ...
    function esEdicionDetalle() {
        const v = $('#tblDetalleSalidaMercancia').data('es-edicion');
    ...
    function inicializarDetalle() {
        ...
        const $tabla = $('#tblDetalleSalidaMercancia');
        ...
        buscadorArticulo = App.autocompletar({
            ...
            endpoint: '/SalidasMercancia/BuscarArticulos',
            ...
            onSeleccion: async a => {
                ...
                // En la salida el costo lo determina siempre el servidor (costo promedio móvil):
                // este campo es de solo lectura y sirve solo como referencia visual. Se muestra el
                // costo promedio del artículo cuando el buscador lo trae; si no, queda en 0. El
                // valor NO afecta al asiento -- el servidor ignora cualquier CostoUnitario posteado.
                ...
                if (almacenDefecto) {
                    const respuestaAlmacen = await $.get('/SalidasMercancia/ObtenerAlmacenPorCodigo', { codigo: almacenDefecto });
        ...
        buscadorAlmacen = App.autocompletar({
            ...
            endpoint: '/SalidasMercancia/BuscarAlmacenes',
    ...
    async function cargarDetalleRemoto() {
        const entry = $('#tblDetalleSalidaMercancia').data('entry');
        const respuesta = await $.get('/SalidasMercancia/ObtenerDetalle', { entry });
    ...
    function pintarDetalle() {
        const $tbody = $('#tblDetalleSalidaMercancia tbody');
    ...
    /** Recalcula el total de la línea: Cantidad x Costo unitario (informativo). */
    ...
    $(document).on('click', '.btn-editar-linea', function () {
        ...
        if (codAlmacen) {
            $.get('/SalidasMercancia/ObtenerAlmacenPorCodigo', { codigo: codAlmacen }).then(respuestaAlmacen => {

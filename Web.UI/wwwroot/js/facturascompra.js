$(function () {
    const tabla = $('#tblFacturasCompra').DataTable({
        ajax: { url: '/FacturasCompra/ObtenerTodos', dataSrc: App.dataSrcTabla },
        columns: [
            { data: 'numDoc' },
    ...
    function inicializarBuscadorSocio() {
        ...
        App.autocompletar({
            ...
            error: $('#CodigoSnError'),
            endpoint: '/FacturasCompra/BuscarSocios',
    ...
    $('#btnNuevo').on('click', async function () {
        const html = await $.get('/FacturasCompra/FormularioCrear');
        abrirModal(html);
    });

    $('#tblFacturasCompra').on('click', '.btn-editar', async function () {
        const entry = $(this).data('entry');
        const html = await $.get('/FacturasCompra/FormularioEditar', { entry });
        abrirModal(html);
    });

    $('#tblFacturasCompra').on('click', '.btn-eliminar', async function () {
        const entry = $(this).data('entry');
        const confirmado = await App.confirmarEliminar(`Se eliminará la factura de compra #${entry}.`);
        if (!confirmado) return;

        const respuesta = await App.eliminar(`/FacturasCompra/Eliminar?entry=${entry}`);
        if (!respuesta.resultado) {
            App.mostrarError(respuesta.mensaje);
            return;
        }
        App.mostrarExito('Factura de compra eliminada correctamente.');
        recargarTabla();
    });

    // --- Serie de numeración para generar el número de documento (solo aplica al crear) ---

    function inicializarSerieFacturaCompra() {
        const $sel = $('#selectSerieFacturaCompra');
        if ($sel.length === 0) return;

        const datosEl = document.getElementById('datosSeriesFacturaCompra');
    ...
    function esSerieManualFacturaCompra() {
        const $sel = $('#selectSerieFacturaCompra');
    ...
    function actualizarNumDocSegunSerie() {
        ...
        if (esSerieManualFacturaCompra()) {
            $numDoc.val('').prop('disabled', false).attr('placeholder', '');
        } else {
            const sigNumero = $('#selectSerieFacturaCompra').find('option:selected').data('sig-numero');
            $numDoc.val(sigNumero ?? '').prop('disabled', true).attr('placeholder', 'Se generará al guardar');
        }
    }

    $(document).on('change', '#selectSerieFacturaCompra', actualizarNumDocSegunSerie);

    $(document).on('click', '#btnGuardarFacturaCompra', async function () {
        const $boton = $(this);
        const esEdicion = $boton.data('edicion') === true || $boton.data('edicion') === 'true';
        const entry = $boton.data('entry');

        if (!esEdicion) {
            const serieSeleccionada = $('#selectSerieFacturaCompra').val();
        ...
        // servidor lo calcula y avanza el consecutivo al registrar la factura de compra (ver
        // FacturaCompraDomain.InsertarAsync en la API), no antes. Para series Manual, el campo #NumDoc
        // está habilitado y su valor viaja normalmente en recolectarFormulario.
        const datos = App.recolectarFormulario('#formFacturaCompra');
        if (!esEdicion) {
            datos.Serie = $('#selectSerieFacturaCompra').val();
        }
        ...
        if (!esEdicion) {
            const respuestaCabecera = await App.enviarJson('/FacturasCompra/Crear', 'POST', datos);
            ...
            for (const linea of lineasLocales) {
                ...
                const respuestaLinea = await App.enviarJson('/FacturasCompra/CrearLinea', 'POST', {
                    ...lineaSinId,
                    Entry: entryCreado
                });
            ...
            if (fallidas > 0) {
                await App.mostrarExito(`Factura de compra creada correctamente. Líneas guardadas: ${exitosas} de ${exitosas + fallidas}.${sufijoNumDoc}`);
            } else {
                await App.mostrarExito(`Factura de compra creada correctamente.${sufijoNumDoc}`);
            }
        ...
        const respuesta = await App.enviarJson(`/FacturasCompra/Editar?entry=${entry}`, 'POST', datos);
        if (!respuesta.resultado) {
            App.mostrarError(respuesta.mensaje);
            return;
        }
        ...
        bootstrap.Modal.getInstance(document.getElementById('modalFormulario')).hide();
        App.mostrarExito('Factura de compra actualizada correctamente.');
        recargarTabla();
    });
    ...
    function esEdicionDetalle() {
        const v = $('#tblDetalleFacturaCompra').data('es-edicion');
    ...
    function inicializarDetalle() {
        ...
        const $tabla = $('#tblDetalleFacturaCompra');
        ...
        buscadorArticulo = App.autocompletar({
            ...
            endpoint: '/FacturasCompra/BuscarArticulos',
            ...
            onSeleccion: async a => {
                ...
                if (almacenDefecto) {
                    const respuestaAlmacen = await $.get('/FacturasCompra/ObtenerAlmacenPorCodigo', { codigo: almacenDefecto });
        ...
        buscadorAlmacen = App.autocompletar({
            ...
            endpoint: '/FacturasCompra/BuscarAlmacenes',
        ...
        buscadorImpuesto = App.autocompletar({
            ...
            endpoint: '/FacturasCompra/BuscarImpuestos',
    ...
    async function cargarDetalleRemoto() {
        const entry = $('#tblDetalleFacturaCompra').data('entry');
        const respuesta = await $.get('/FacturasCompra/ObtenerDetalle', { entry });
    ...
    function pintarDetalle() {
        const $tbody = $('#tblDetalleFacturaCompra tbody');
    ...
    $(document).on('click', '.btn-editar-linea', async function () {
        ...
        if (codAlmacen) {
            const respuestaAlmacen = await $.get('/FacturasCompra/ObtenerAlmacenPorCodigo', { codigo: codAlmacen });
        ...
        if (codigoImpuesto) {
            const respuestaImpuesto = await $.get('/FacturasCompra/ObtenerImpuestoPorCodigo', { codigo: codigoImpuesto });
    ...
    $(document).on('click', '.btn-eliminar-linea', async function () {
        ...
        if (esEdicionDetalle()) {
            const entry = $('#tblDetalleFacturaCompra').data('entry');
            const respuesta = await App.eliminar(`/FacturasCompra/EliminarLinea?entry=${entry}&noLinea=${clave}`);
            if (!respuesta.resultado) {
                App.mostrarError(respuesta.mensaje);
                return;
            }
    ...
    $(document).on('click', '#btnGuardarLinea', async function () {
        ...
        if (esEdicionDetalle()) {
            ...
            const url = esEdicionLinea
                ? `/FacturasCompra/EditarLinea?entry=${entry}&noLinea=${noLineaOriginalEnEdicion}`
                : '/FacturasCompra/CrearLinea';

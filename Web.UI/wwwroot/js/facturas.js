$(function () {
    ...
    $(document).on('click', '#btnCancelarDocFactura', async function () {
        ...
        try {
            const respuesta = await App.enviarJson(`/Facturas/Editar?entry=${entry}`, 'POST', { Cancelado: 'S' });
            if (!respuesta.resultado) {
                App.mostrarError(respuesta.mensaje);
                return;
            }
    ...
    function actualizarNumDocSegunSerie() {
        ...
        if (esSerieManualFactura()) {
            ...
            const sigNumero = $('#selectSerieFactura').find('option:selected').data('sig-numero');
    ...
    $(document).on('click', '#btnGuardarFactura', async function () {
        ...
        if (!esEdicion) {
            ...
            const respuesta = await App.enviarJson('/Facturas/Crear', 'POST', datos);
            if (!respuesta.resultado) {
                App.mostrarError(respuesta.mensaje);
                return;
            }
    ...
    function inicializarDetalle() {
        ...
        buscadorArticuloCod = App.autocompletar({
            ...
            endpoint: '/Facturas/BuscarArticulosPorCodigo',
    ...
    async function aplicarAlmacenDefectoDeArticulo(a) {
        ...
        const r = await $.get('/Facturas/ObtenerAlmacenPorCodigo', { codigo: cod });
